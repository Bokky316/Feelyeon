 ...
 * - build.gradle에 testCompileOnly, testAnnotationProcessor에 롬복 테스트 의존성 추가
...
public class ItemRepositoryTest {
    ...
    public void saveItemTest(){
        ...
        Item item = Item.builder()
                ...
                //.regTime(LocalDateTime.now())   // regTime을 명시적으로 설정하지 않아도 JPA Auditing이 자동으로 값을 넣습니다
                .build();
    ...
    public void findAllItemsTest(){
        ...
        List<Item> items = itemRepository.findAll(); // findAll() : ListCrudRepository 인터페이스의 메서드

        // Then : 저장된 Item 엔티티의 개수가 10개인지 검증
        assertThat(items).isNotEmpty();  // 1차 검증(단정)
        assertThat(items.size()).isGreaterThan( 10);  // 2차 검증
        // 저장된 값 확인
    ...
    public void findItemByIdTest(){
        ...
        // When
        // findById() : Item 엔티티의 ID로 Item 엔티티를 조회
    ...
    public void updateItemTest(){
        ...
        // 구체적으로 영속영역에 있는 엔티티를 수정하면 오염되고 나중에 트랜잭션 끝나는 시점에 자동 반영
        ...
        // Then : 수정된 Item 엔티티의 상품명이 수정된 상품명과 같은지 검증
    ...
    public void deleteItemTest() {
        ...
        Long itemId = savedItem.getId();    // 저장된 Item 엔티티의 ID 저장

        // When
        itemRepository.deleteById(itemId);  // 위에서 저장한 Item 엔티티의 ID로 삭제

        // Then
        Optional<Item> deletedItem = itemRepository.findById(itemId);
        assertThat(deletedItem).isNotPresent(); // 삭제된 Item 엔티티가 존재하지 않는지 검증
    ...
    public void findByItemNmOrItemDetailTest(){
        ...
        List<Item> itemList = itemRepository
                .findByItemNmOrItemDetail("테스트 상품 1", "테스트 상품 상세 설명 5");
...
//    }
    ...
    public void findItemsByStatusAndDetailTest() {
        ...
        // When
        // queryFactory의 역할 : JPAQuery(쿼리) 객체를 생성하여 QueryDSL을 사용하여 쿼리를 작성
        // selectFrom() : 조회할 엔티티를 지정, 매개변수 QItem.item으로 Item 엔티티를 조회
        // where() : 조회 조건을 지정, 매개변수로 조건을 지정, 매개변수: 조건식
        // fetch() : 조회 결과를 반환, 조회 결과를 List로 반환
        List<Item> items = queryFactory
                .selectFrom(qItem)                               // 조회할 엔티티 지정
                .where(
                        qItem.itemSellStatus.eq(status),         // 상태가 SELL인 상품
                        qItem.itemDetail.contains(searchDetail)  // AND 조건, 상세 설명에 특정 키워드 포함(LIKE)
                )
    ...
    public void findItemsWithConditionsTest() {
        ...
        // builder.and(조건식) : 첫번째 AND 조건을 추가, 이 조건은 첫번째 조건과 그 다음에 오는 조건의 연결.조건 한개면 무의미
        // 두 번째 and() : 두번째 조건과 세 번째 조건을 AND로 연결
        // 세 번째 and() : 세번째 조건과 네 번째 조건을 AND로 연결, 이때 네번째 조건이 없으면 무의미
    ...
    public void findItemsWithConditionsTest2() {
        ...
        log.info("총 아이템 수: " + itemPageResult.getTotalElements());
    ...
    private Item saveTestItem() {
        ...
        Item item = Item.builder()
                ...
                .itemDetail("테스트 상품 상세 설명")
                .itemSellStatus(ItemSellStatus.SELL)
                //.regTime(now) //regTime을 명시적으로 설정하지 않아도 JPA Auditing이 자동으로 값을 넣습니다
                //.updateTime(now)  //updateTime을 명시적으로 설정하지 않아도 JPA Auditing이 자동으로 값을 넣습니다
                .build();
